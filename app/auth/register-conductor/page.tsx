...
export default function RegisterConductorPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            // 1) Crear conductor en Auth + usuario + conductor vía endpoint específico
            const res = await fetch('/api/auth/register-conductor', {
                method: 'POST',
            ...
            if (!res.ok) {
                const { error } = await res.json().catch(() => ({ error: 'No se pudo crear la cuenta de conductor.' }));
        ...
        <div className="min-h-screen bg-[#f6f8fa] flex flex-col items-center justify-center">
            ...
            <div className="w-full max-w-xl bg-white rounded-xl shadow-lg p-10 flex flex-col items-center">
                <div className="text-center mb-6">
                    <h2 className="text-2xl font-bold mb-1">Registro de Conductor</h2>
                    <p className="text-sm text-gray-500 mb-4">
                        Completa tus datos para crear tu cuenta de conductor
                    </p>
                    <div className="flex items-center justify-center space-x-2 text-xs text-blue-600 bg-blue-50 px-3 py-1 rounded-full">
                        <Car className="w-4 h-4" />
                        <span>Permite reservar estacionamientos y gestionar vehículos</span>
                    </div>
                </div>
                ...
                >
                    <svg width="20" height="20" viewBox="0 0 48 48" className="mr-2">
                        ...
                        <path fill="#EA4335" d="M24 48c6.64 0 12.53-2.36 17.13-6.45l-7.19-5.59c-2.01 1.35-4.58 2.14-7.44 2.14-6.19 0-11.47-3.63-13.25-8.86l-8.06 6.27C6.7 42.42.26 14.86 48 24 48z" />
